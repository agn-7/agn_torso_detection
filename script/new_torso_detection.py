...
class TorsoDetection:
    ...
    def __init__(self):   
        ...
        rospy.Subscriber("/scan_ubg", LaserScan, self.upLaserCb, None, 10 ) #laser e balaE madde nazar ast
        
        self.torsoPublisher = rospy.Publisher("/TorsoDetectionRange", std_msgs.msg.Float32, queue_size = 1)
        self.torsoPublisher2 = rospy.Publisher("/TorsoDetectionPoint", std_msgs.msg.Float32, queue_size = 1)
        self.markerPublisher = rospy.Publisher("/TorsoMarker", visualization_msgs.msg.MarkerArray, queue_size = 10)
        
        ##self.torsoPublisherScenario = rospy.Publisher('TorsoDetected', std_msgs.msg.Bool, queue_size = 10) #for scenario
    ...
    def upLaserCb(self, laser):  
        if self.torsoPublisher.get_num_connections() > 0 : #for scenario used 
            ...
            while i < localEndPoint:        #clustring                  
                ...
                if (Distance < self.threshold) and (laser.ranges[i] < self.outOfRange ) and (laser.ranges[i] != 0):
                    clustersPoints[len(clustersPoints) - 1].append(i)
                    appendFlag = True  
                    
                elif (Distance > self.threshold ) and (appendFlag == True) and (laser.ranges[i] < self.outOfRange ) and (laser.ranges[i] != 0) :
                    clustersPoints.append([])
                    appendFlag = False 
            ...
            while i < len(clustersPoints) - 1:        #better clustring # -1 baraye inke y duneye akhari hamishe xalie                      
            ...
            while i < len(clustersPoints):     #clusters Distance
               ...
               indexes.append(clustersPoints[i][middle])   #index haye miani #pointhaye mianie cluster ha
            ...
            while i < len(clustersDistance):   #good distance choose
                if clustersDistance[i] > self.minLenght and clustersDistance[i] < self.maxLenght:
                    ...
                    goodMarkers.append(markIndexes[i]) # index haye xub dar clustersPoints.  ya indexe clusterhaye xub    
                    j = 0
            ...
            while i < len(goodMarkers): #curv detection
                ...
                while j < len(clustersPoints[gm]) - 2:    #baraye inke avvali va akhari dar bala hesab shode and.                                                       
                    ...
                    n3 = ( (clustersPoints[gm][len(clustersPoints[gm]) - 1]) - (clustersPoints[gm][j + 1]) )
                    ...
                    angl = numpy.arccos((pow(dist1[i][j], 2) + pow(dist2[i][j], 2) - pow(dist3[i], 2)) / (2 * dist1[i][j] * dist2[i][j]) )
            ...
            while i < len(angle):            
                ...
                while j < len(angle[i]) - 1:
                    ...
                    if (abs(angle[i][j] - angle[i][j + 1]) < 25) and (abs(angle[i][j] - self.threshold_angle) < 25) :                                       
                        cmp2 = cmp2 + 1
                                                
                    if ((len(angle) - cmp3) == 0) and (abs(angle[i][j + 1] - self.threshold_angle) < 25): #baraye inke zaviye akhar ham barresi shavad
                        cmp2 = cmp2 + 1                                                
                                                                                                                                          
                    j = j + 1
                    
                if (len(angle[i]) - cmp2) < ((20.0 * len(angle[i])) / 100): #yani dar suraT varede if mishavad k ekhtelafe cmp2 ba har goodcluster bayad kamtar az 20% an bashad, yany 80% sahih bashad                             
    ...
    #         #print("goods", goodMarkers)         
            ...
            if len(goodIndexes) > 0:            
                bestIndex = goodIndexes[minIndex]   #bhtarin shot. k shote mianie aan cluster ast. # be manie [j] ast
            ...
            while i2 < len(curv_indexes):
                 ...
                 #x2.append([])
                 #y2.append([])
                 lif = clustersPoints[curv_indexes[i2]] # LIF means, is, Legs Indexes First                         
